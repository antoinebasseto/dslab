...
def create_dataset_cell_ndarray(frames, channels, image, droplet_table_path, cell_table_path, allFrames = False, allChannels = False, buffer = 3, suppress_rest = True, suppression_slack = 1, discard_boundaries = False, omit_patches = False, median_filter_preprocess = False):
    droplet_table = pd.read_csv(droplet_table_path, index_col = False)
    cell_table = pd.read_csv(cell_table_path, index_col = False)
